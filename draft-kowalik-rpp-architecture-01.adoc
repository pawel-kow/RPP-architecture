...
:docnumber: draft-kowalik-rpp-architecture-01
...
* **Extension mechanisms:** Providing mechanisms for creating new resource types and for extending existing resource types with new data elements or operations.
...
RPP utilizes both HTTP status codes and RPP-specific status codes, using RPP-specific HTTP Headers. The response body may contain machine readable problem details <<RFC7807>> for detailed status reporting. Using HTTP status codes and headers allows the client or an intermediate (HTTP status only) to determine what action to take based on status code and header details only.
...
*  Definition of RPP-specific status codes, warnings of additional processing information, provided in the response, preferably outside of resource representation (e.g. in HTTP Headers) to give granular information about provisioning request status. 
...
* Both HTTP and RPP-specific status codes are used to determine if the outcome of a request is successful or not.
...
The Resource Definition Layer is designed to be extensible, allowing for the addition of new resource types, data elements, and operations without breaking existing implementations. This extensibility is achieved through:
...
* Added Extension mechanisms section to Resource Definition Layer
== References
