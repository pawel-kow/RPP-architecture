...
==== Collision Avoidance and Namespace Management

RPP extensions MUST define a unique namespace to prevent conflicts with other extensions and with core protocol elements. This namespace is used consistently in resource identifiers, data element identifiers, and URL path segments. Identifiers are constructed using a namespace prefix and a local name, for example: `myextension_domainname`, where `myextension` is the namespace prefix. The separator character should be compatible with common programming languages; use of the "." (dot) is discouraged for this reason.

Standardized RPP extensions MUST register their namespace prefix with IANA to ensure global uniqueness and avoid collisions. Private (non-standardized) extensions are also required to use a namespace prefix, but registration with IANA is not mandatory. However, private extensions SHOULD choose a prefix that is unlikely to conflict with other extensions or with RPP core elements.
...
TODO: see RDAP discussion about bare identifiers, need to make a choice here also.
