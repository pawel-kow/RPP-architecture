...
RPP will define a mechanism for object-level authorization, preventing unauthorized access to specific objects or resources. Each object will have an associated sponsor or owner, and the protocol will allow for the specification of which clients are authorized to access or modify that object. This could be achieved through a combination of authentication tokens, scopes, and resource-specific permissions.
