...
The schemas will be constructed in both strict variant not allowing additional unknown properties, but also in lenient variant ignoring those. The schema language must support schema composition through modular definitions and references to enable extensibility at the allowed extension points.
