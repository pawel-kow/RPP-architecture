...
**Encapsulation** and **Data Structure** are sub-layers of Data Representation and described later in this document. They focus on the encapsulation and structure of RPP messages, respectively.
...
//PK: How would routing be reflected in the protocol architecture? I think it is more of an implementation detail.
...
//PK: Not sure what you mean by "mixing"? I think we should define RPP-specific error codes and relate them to HTTP error codes for consistency. What I would like to avoid is that GET for a domain which does not exist returns 200 and a different RPP-Error header with specific error code. I would rather see 404 with a specific RPP-Error header.
...
//PK: I think encapsulation is important as it defines how the data is wrapped and transported. It is a separate concern from the data structure. As mentioned in the E-mail the same structure and format may be represented in different encapsulations for (future) use-cases so keeping it as extension point may be beneficial. On the other side it may be all compressed into Data Format definition. I don't have super strong feelings here.
...
//PK: we discussed this in context of OAuth. There would be a value to define some common scopes for RPP, so that interoperability would be easier. Even if we just say we have rpp:read and rpp:write scopes.
...
// PK: after some more in-depth lecture I have my doubts if overloading HEAD is a good idea. Semantically in EPP CHECK is not same as INFO without body. rfc2616 says that HEAD should be identical to GET except for the body. My concern is that many frameworks in fact do an implicit implementation of HEAD and also the responses to HEAD may influence the caching mechanisms and middle boxes.
// What about thinking of an alternative for check command? Maybe a POST/PUT with Expect header? It also does not return any body.
...
* **Plain JSON:** Standard JSON format <<RFC8259>>. 
